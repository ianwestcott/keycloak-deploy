...
awsenv=$1
appenv=$APP-$awsenv
aws="docker run -i --rm -v $HOME/.aws:/root/.aws -v /tmp:/tmp/local-tmp -i amazon/aws-cli"
...
pushd $githash
zip -r /tmp/$githash.zip .
popd

rm -r $githash

# Put zip on S3 and use to create application version
$aws s3 cp /tmp/local-tmp/$githash.zip s3://$S3_BUCKET_NAME/$APP/$githash.zip
...
while [[ `$aws elasticbeanstalk describe-environments --environment-names $appenv | grep '"Status"' | cut -d: -f2  | sed -e 's/^[^"]*"//' -e 's/".*$//'` = "Updating" ]]; do sleep 10; printf "."; done
if [[ `$aws elasticbeanstalk describe-environments --environment-names $appenv | grep VersionLabel | cut -d: -f2 | sed -e 's/^[^"]*"//' -e 's/".*$//'` = $githash ]]; then echo "The version of application code on Elastic Beanstalk matches the version that Semaphore sent in this deployment."; echo "Your environment info:"; $aws elasticbeanstalk describe-environments --environment-names $appenv; else echo "The version of application code on Elastic Beanstalk does not match the version that Semaphore sent in this deployment. Please check your AWS Elastic Beanstalk Console for more information."; echo "Your environment info:"; $aws elasticbeanstalk describe-environments --environment-names $appenv; false;
fi
sleep 5
a="0"
echo "Waiting for environment health to turn Green"
while [[ `$aws elasticbeanstalk describe-environments --environment-names $appenv | grep '"Health":' | cut -d: -f2  | sed -e 's/^[^"]*"//' -e 's/".*$//'` != "Green" && $a -le 30 ]]; do sleep 2; a=$[$a+1]; printf "."; done
if [[ `$aws elasticbeanstalk describe-environments --environment-names $appenv | grep '"Health":' | cut -d: -f2 | sed -e 's/^[^"]*"//' -e 's/".*$//'` = "Green" ]]; then echo "Your environment status is Green, congrats!"; else echo "Your environment status is not Green, sorry."; false; fi;
